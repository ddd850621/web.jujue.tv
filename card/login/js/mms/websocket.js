...
$.wsock = {
    ...
    sendSocketMsg: function(data, callback) {
        try {
            console.log('websocket send data=%s', JSON.stringify(data));
            if ( $.wsock.wsState == 'conn'){
                if ( typeof data == 'object' ){
                    socket.emit('request', data, function(reply){
                        if ( typeof reply == 'object')
                            console.log('websocket send reply=%s', JSON.stringify(reply));
                        else
                            console.log('websocket send reply=%s', reply);
