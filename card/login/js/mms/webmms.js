...
function MMS(app, ws) {
    ...
    this.CallMMS = function(target, func, args, cb){
        ...
        $.mbus.XrpcCall(target, func, args,
            function(reply) {
                if (typeof reply == 'object') console.log('CallMMS: reply=%s', JSON.stringify(reply));
                else console.log('CallMMS: reply=%s', reply);
                if ( typeof cb == 'function' ) cb(reply);
            }
        );
        */
    ...
    var _GetQPin = function( cb ){
        if ( dcStat == 'conn' ){
            if ( SToken!= '' )
                $.mbus.GetQPin( SToken, cb );
            else {
                if ( typeof cb == 'function' ){
                    var ret = {"ErrCode":mmserr.NullSToken.code,"ErrMsg":mmserr.NullSToken.msg};
                    cb(ret);
...
$.mbus = {
    ...
    UnregToCenter: function( SToken, cb ){
    ...
    GetQPin: function( SToken, cb ) {
        var rdata = {"SToken":SToken};
        var req = {"func":"getqpin","data":rdata};
        console.log('GetQPin: req=%s', JSON.stringify(req));
        $.wsock.sendSocketMsg(req,
            function(reply){
                console.log('webmms:GetQPin result=%s', JSON.stringify(reply));
                if ( typeof cb == 'function' ) cb(reply);
            }
        ); 
    },
