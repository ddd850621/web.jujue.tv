...
var mmsSendXmsgNodeRed = function (target, data) {
	 var cookies = document.cookie.split(";");
	 var dataNodeRed = { triggerID: "demo", targets: [ cookies[cookies.length-1].trim() ]};
	 //console.log('cookie:' + cookies[cookies.length-1]);
	 //console.log('mmsSendXmsg:'+ target + '::::' + data);
	 console.log(cookies[cookies.length-1].trim());
	 mms.SendMMS(target,dataNodeRed,function cb(reply){console.log(reply);});
}
